...
// Modal for selecting which occurrence of text to comment on
class TextSelectionModal extends Modal {
  constructor(
    app: App,
    private readonly selectedText: string,
    private readonly occurrences: number[],
    private readonly nodeNote: string,
...
export class MindmapView extends ItemView {
  ...
  private static readonly MIN_DRAWER_WIDTH = 200;
  ...
  async onOpen(): Promise<void> {
    ...
    this.noteCommentsPanelEl = this.drawerEl.createDiv({ cls: "mindmap-comments-panel is-hidden" });
    const headerEl = this.noteCommentsPanelEl.createDiv({ cls: "mindmap-comments-header" });
    headerEl.createEl("h3", { cls: "mindmap-comments-title", text: "评论" });
    const closeBtn = headerEl.createEl("button", { cls: "mindmap-comments-close", text: "×" });
    closeBtn.type = "button";
    closeBtn.addEventListener("click", () => {
      this.hideCommentsPanel();
    });
    this.noteCommentsPanelEl.createDiv({ cls: "mindmap-comments-list" });
  ...
  private async renderMarkdown(markdown: string): Promise<void> {
    ...
    renderedFootnoteSpans.forEach((span, index) => {
      console.log(`[DEBUG] Footnote span ${index}:`, span.outerHTML.substring(0, 100));
    });
    ...
    this.notePreviewEl.addEventListener("pointerup", () => {
      console.log('[DEBUG] notePreviewEl pointerup event triggered');
      const selection = window.getSelection();
      console.log('[DEBUG] Selection at pointerup:', selection?.toString());
      window.setTimeout(() => {
        console.log('[DEBUG] Calling updateNoteSelectionToolbar after timeout');
        this.updateNoteSelectionToolbar();
      }, 10);
    });
  ...
  private findAndHighlightTextByPosition(
    ...
    if (offsetInNode + searchText.length <= content.length) {
      ...
      console.log('[DEBUG] Text spans multiple nodes, skipping complex case');
      console.log('[DEBUG] === findAndHighlightTextByPosition END ===');
      // For now, skip highlighting if text spans multiple nodes
      return false;
    }
  }
  ...
  private jumpToComment(commentId: string): void {
    ...
    setTimeout(() => {
      ...
      if (commentCard) {
        ...
        commentCard.addClass('mindmap-comment-card-highlight');
        setTimeout(() => {
          commentCard.removeClass('mindmap-comment-card-highlight');
        }, 2000);
      }
    }, 100);
  }
    ...
    if (this.noteSurfaceEl?.hasClass("is-editing")) {
      ...
      if (this.noteEditorView && !this.isMobileLayout) {
        ...
        if (position !== -1) {
          ...
          console.log('[DEBUG] Footnote marker not found in editor');
          new Notice("未找到脚注标记");
        }
      } else {
        // Textarea editor
        ...
        if (position !== -1) {
          ...
          setTimeout(() => {
            input.removeClass('mindmap-footnote-flash');
          }, 1500);
        } else {
        ...
        if (node && node.note) {
          ...
          if (hasFootnote) {
            ...
            setTimeout(() => {
              ...
              markerSpan = this.notePreviewEl?.querySelector(`span.mindmap-footnote-marker[data-footnote-id^="${footnoteId}"]`) as HTMLElement | null;
              console.log('[DEBUG] Marker found after re-render:', !!markerSpan);
              if (markerSpan) {
                console.log('[DEBUG] Found marker with ID:', markerSpan.dataset.footnoteId);
              }
              ...
              if (markerSpan) {
                ...
                console.log('[DEBUG] Still not found, trying again with more delay');
                setTimeout(() => {
                  // Use attribute starts-with selector to handle potential suffixes added by Obsidian
                  markerSpan = this.notePreviewEl?.querySelector(`span.mindmap-footnote-marker[data-footnote-id^="${footnoteId}"]`) as HTMLElement | null;
                  console.log('[DEBUG] Marker found after second retry:', !!markerSpan);
                  if (markerSpan) {
                    console.log('[DEBUG] Found marker with ID:', markerSpan.dataset.footnoteId);
                  }
                  ...
                  if (markerSpan) {
                    ...
                    console.log('[DEBUG] Still not found after second retry');
                    new Notice("未找到脚注标记，请尝试刷新页面");
                  }
                }, 300);
              }
            }, 400);
  ...
  private flashEditorPosition(startPos: number, length: number): void {
    ...
    console.log('[DEBUG] Flashing editor position:', startPos, 'length:', length);
  }
  ...
  private hasActiveNoteSelection(): boolean {
    ...
    if (!this.noteSurfaceEl?.hasClass("is-editing")) {
      ...
      if (selection && !selection.isCollapsed && selection.toString().trim().length > 0) {
        ...
        console.log('[DEBUG] Selection in preview:', isInPreview);
        return isInPreview;
      }
  ...
  private updateNoteSelectionToolbar(): void {
    ...
    } else if (!this.noteSurfaceEl?.hasClass("is-editing")) {
      // Preview mode: use window selection range to get coordinates
      const selection = window.getSelection();
      if (selection && !selection.isCollapsed && selection.rangeCount > 0) {
        const range = selection.getRangeAt(0);
        const rect = range.getBoundingClientRect();
        left = rect.left - surfaceRect.left + rect.width / 2;
  ...
  private handleAddComment(): void {
    ...
    if (this.noteSurfaceEl?.hasClass("is-editing")) {
      ...
      if (this.noteEditorView && !this.isMobileLayout) {
        // Edit mode with CodeMirror
        console.log('[DEBUG] Handling CodeMirror editor mode comment');
        const selection = this.noteEditorView.state.selection.main;
        if (selection.empty) {
          console.log('[DEBUG] Empty selection in editor');
          this.hideNoteSelectionToolbar();
          return;
        }
        selectedText = this.noteEditorView.state.doc.sliceString(selection.from, selection.to);
        selectionStart = selection.from;
      } else {
        // Edit mode with textarea
        console.log('[DEBUG] Handling textarea mode comment');
        const input = this.noteInputEl;
        const start = input.selectionStart ?? 0;
        const end = input.selectionEnd ?? 0;
        if (start === end) {
          console.log('[DEBUG] Empty selection in textarea');
          this.hideNoteSelectionToolbar();
          return;
        }
    ...
    const modal = new CommentModal(this.app, selectedText, (comment: string) => {
      ...
      if (isEditMode) {
        ...
        if (this.noteEditorView && !this.isMobileLayout) {
          ...
          const view = this.noteEditorView;
          view.dispatch({
            changes: { from: selectionStart, to: selectionStart + selectedText.length, insert: footnoteMarker },
            scrollIntoView: true
          });
          view.focus();
        } else {
          // Textarea editor
          const input = this.noteInputEl;
          const before = input.value.substring(0, selectionStart);
          const after = input.value.substring(selectionStart + selectedText.length);
          input.value = before + footnoteMarker + after;
          input.setSelectionRange(selectionStart + footnoteMarker.length, selectionStart + footnoteMarker.length);
          input.dispatchEvent(new Event("input"));
          input.focus({ preventScroll: true });
        }
        ...
        // In preview mode, replace the selected text in node.note
        if (!node.note) {
          console.log('[DEBUG] Node note is empty in preview mode');
          return;
        }
        const before = node.note.substring(0, selectionStart);
      ...
      const newComment: MindmapComment = {
        ...
        position: selectionStart, // Keep for backward compatibility
  ...
  private showCommentsPanel(): void {
  ...
  private hideCommentsPanel(): void {
  ...
  private deleteComment(node: MindmapNode, commentId: string): void {
  ...
  private renderCommentsList(node: MindmapNode): void {
    const listEl = this.noteCommentsPanelEl.querySelector(".mindmap-comments-list") as HTMLElement;
    if (!listEl) {
      return;
    }
  ...
  private renderCommentCard(containerEl: HTMLElement, node: MindmapNode, comment: MindmapComment): void {
    ...
    cardEl.addEventListener("click", (e) => {
      ...
      if ((e.target as HTMLElement).closest(".mindmap-comment-action-btn")) {
        return;
      }
      ...
      if (comment.footnoteId) {
        ...
        // Legacy mode: use position-based highlighting
  ...
  private renderCommentEditForm(cardEl: HTMLElement, node: MindmapNode, comment: MindmapComment): void {
  ...
  private updateComment(node: MindmapNode, commentId: string, newComment: string): void {
    const comment = node.comments?.find(c => c.id === commentId);
    if (comment) {
  ...
  private prepareMarkdownForPreview(markdown: string): string {
    ...
    let processed = markdown.replace(/!\[\[([^\]]+)\]\]/g, (full, rawTarget: string) => {
    ...
    processed = processed.replace(/\[\^([a-zA-Z0-9]+)::([^\]]+)\]/g, (match, footnoteId, originalText) => {
      // Create a clickable highlighted span for the original text
      return `<span class="mindmap-footnote-marker" data-footnote-id="${footnoteId}" title="点击查看评论">${originalText}</span>`;
    });
    return processed;
  }
  ...
  private updateDrawerWidth(): void {
    ...
    this.drawerWidth = Math.max(MindmapView.MIN_DRAWER_WIDTH, this.drawerWidth);
  ...
  private startDrawerResize(event: PointerEvent): void {
    ...
    const onMove = (moveEvent: PointerEvent): void => {
      ...
      // Only enforce minimum width, no maximum limit
      this.drawerWidth = Math.max(MindmapView.MIN_DRAWER_WIDTH, startWidth + delta);
      this.updateDrawerWidth();
      this.updateDrawerWidth();
    };
  ...
  private async openInternalTarget(path: string, inNewTab: boolean, focusLinkedFromPath = this.file?.path ?? null): Promise<void> {
    ...
    if (isMindmap) {
      ...
      if (targetLeaf === this.leaf) {
        ...
        const targetView = targetLeaf.view as MindmapView;
        if (Platform.isMobile) {
          targetView.setZenMode(true);
        }
        if (normalizedFocusLinkedFromPath) {
          targetView.focusLinkedNodeFromPath(normalizedFocusLinkedFromPath);
        }
      }
      return;
    }
